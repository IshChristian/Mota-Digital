...
export default function SavingsScreen() {
  ...
  const checkConsent = async () => {
    try {
      ...
      const hasLending = consents.some(
        (c: any) => c.consentType === "lending_terms",
      );
      const hasSavings = consents.some(
        (c: any) => c.consentType === "savings_terms",
      );
  ...
  const handleTransaction = async () => {
    ...
    try {
      ...
      Alert.alert(
        "Error",
        err.response?.data?.message || `Failed to ${modalType}`,
      );
    } finally {
      setSubmitting(false);
    }
  ...
  return (
    ...
    >
      ...
      >
        <DriverHeader
          back
          title="Keep Me in Bank"
          subtitle="Build savings from your verified driver earnings."
        />
        ...
        <View style={s.trackerCard}>
          ...
          <View style={s.progressBarBg}>
            <View
              style={[
                s.progressBarFill,
                { width: `${Math.min((daysHeld / 30) * 100, 100)}%` },
              ]}
            />
          </View>
          ...
          {rewardEligible ? (
            <View style={s.rewardEligibleBox}>
              ...
              <Text style={s.rewardEligibleText}>
                Your funds are currently earning 2% monthly interest!
              </Text>
            </View>
          ) : (
            <Text style={s.rewardPendingText}>
              Hold funds for 30 days to earn 2% interest.
            </Text>
          )}

          <View style={s.accruedBox}>
            <Text style={s.accruedLabel}>Accrued Reward</Text>
            <Text style={s.accruedAmount}>
              +{rewardAccrued.toLocaleString()} RWF
            </Text>
          </View>
        </View>
      </ScrollView>
